...
const ViewConsul = () => {
    ...
    const handleCancel = async (item) => {
        const response = await fetch(`https://consult-sched-production.up.railway.app/api/appointments/cancel/${item._id}`, {
            method: 'PUT',
            headers: {
                'x-auth-token': `${user.token}`
            }
    ...
    const handleDelete = async (consulId) => {
        const response = await fetch(`https://consult-sched-production.up.railway.app/api/appointments/delete/${consulId}`, {
            method: 'DELETE',
            headers: {
                'x-auth-token': `${user.token}`
            }
